	...
	<body style="background-image: url(../res/otherback.jpg); background-size: cover;">
		...
		<div class="container" style="text-align: center; min-width: 100%;">
			...
			<%
				...
				ResultSet getEventsRSET = getEventsSTAT.executeQuery("select e.id, e.topic, e.event_date, e.start_time, e.end_time, e.booked_on, c.name, e.venue from event e, contact c where e.id like '" + request.getParameter("wing") + "%' and e.id = c.event_id and c.role = 'Booker' and e.event_date >= (now() + INTERVAL 4 DAY) and e.status in (1, 2)");
			...
			<table class="table table-bordered" style="white-space: nowrap;">
				...
				while(getEventsRSET.next()){
					...
					out.println("<td><button class=\"btn btn-danger\" onclick=\"feedVal('eventId', '" + getEventsRSET.getString(1) + "', 'event');\"><strong>CANCEL</strong></button></td>");
					out.println("</tr>");
				}
...
<form name="event" id="event" action="confirmation.jsp" method="post" autocomplete="off">
	<input type="hidden" id="eventId" name="eventId">
	<input type="hidden" id="status" name="status" value="can">
</form>