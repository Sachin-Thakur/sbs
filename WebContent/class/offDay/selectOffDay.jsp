				...
				List<TimeSlot> exceptSlots = new ArrayList<TimeSlot>();
					...
					for(int i=0; i<workingSlots.size(); i++){
						if(workingSlots.get(i).getDayOfWeek() == 7 && workingSlots.get(i).getHour()>12){
							...
							for(int j=0; j<holSlots.size(); j++){
								if(workingSlots.get(i).toString().equals(holSlots.get(j).toString())){
									workingSlots.get(i).setAvailability(false,0);
									break;
								}
							}
							for(int j=0; j<bookSlots.size(); j++){
								if(workingSlots.get(i).toString().equals(bookSlots.get(j).toString())){
									workingSlots.get(i).setAvailability(false,1);
									break;
								}
							}
