...
getBookedEventRSET = getBookedEventSTAT.executeQuery("select event_date,start_time from class_events where class_topics_id = "+id +" and status in(1,2)" );
getExceptionSTAT = connection.createStatement();
getExceptionRSET =   getExceptionSTAT.executeQuery("select  exception_date,exception_start_time from class_exceptions ");
...
getDeptQuotaRSET = getDeptQuotaSTAT.executeQuery("select * from department");
 ...
 Date d1  = getTopicDataRSET.getDate(5);
...
int hours =getTopicDataRSET.getInt(4);
...
while(getBookedEventRSET.next()){
	...
	TimeSlot sl = new TimeSlot(Integer.parseInt(getBookedEventRSET.getString(1).substring(0,4)),
		 Integer.parseInt(getBookedEventRSET.getString(1).substring(5, 7)),
		 Integer.parseInt(getBookedEventRSET.getString(1).substring(8)),
		 Integer.parseInt(getBookedEventRSET.getString(2).substring(0, 2)),
		 Integer.parseInt(getBookedEventRSET.getString(2).substring(3, 5)));

	eventExceptSlots.add(sl);
}
...
for(int i=0; i<workingSlots.size(); i++){
	if(workingSlots.get(i).getDayOfWeek() == 7 && workingSlots.get(i).getHour()>12){
		...
		for(int j=0; j<eventExceptSlots.size(); j ++){
			if(workingSlots.get(i).toString().equals(eventExceptSlots.get(j).toString())){
				workingSlots.get(i).setAvailability(false,1);// 1 is passed for for events 
...
%>
		...
		<p ><h3>Topic Details:</h3> <%=getTopicDataRSET.getString(3) %></p>
<table class="table table-bordered style="white-space: nowrap;">
					...
					<%
						for(int i=9; i<17; i += hours){
							out.println("<tr>");
							for(int j=0; j<workingSlots.size(); j ++){
									if(workingSlots.get(j).getHour() == i){
										out.println(workingSlots.get(j).getSlot(hours));
								}
							}
							out.println("</tr>");
						}
...
<%} %>
					...
					<input type = "hidden" id = "hours" name="hours" values= "<%=hours %>">
				</form>
			</div>
		</div>
